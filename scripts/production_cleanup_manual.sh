#!/bin/bash
# production_cleanup_manual.sh - Manual cleanup for moltbot-helpers production deployment

set -e  # Exit on any error

echo "Starting moltbot-helpers production cleanup (manual)..."

# Remove cache files we can access
...
if [ -f "pyproject.toml.production" ]; then
    cp pyproject.toml.production pyproject.toml
    echo "Updated pyproject.toml to production version"
else
    echo "pyproject.toml.production not found, skipping"
fi

# Create production-ready README
if [ -f "README.PRODUCTION" ]; then
    cp README.PRODUCTION README.md
    echo "Updated README.md to production version"
else
    echo "README.PRODUCTION not found, skipping"
fi
