"""
PostgreSQL Unified Interface Module for OpenClaw Integration
...
# Import the PostgreSQL MemoryManager
try:
    from ..memory.memory_manager_pg import MemoryManager
except (ImportError, ValueError):
    # Handle relative import issue when running directly
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
    from memory.memory_manager_pg import MemoryManager

# Import the PostgreSQL TriggerManager
try:
    from ..automation.trigger_manager_pg import TriggerManager
except (ImportError, ValueError):
    # Handle relative import issue when running directly
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))
    from automation.trigger_manager_pg import TriggerManager
...
class UnifiedInterface:
    ...
    project tracking, and enhanced memory management using PostgreSQL.
    """
    
    def __init__(self, db_host=None, db_port=None, db_name=None, db_user=None, db_password=None):
        """Initialize the unified interface with PostgreSQL connection parameters."""
    ...
    def fp_query(self, args: List[str]) -> Dict[str, Any]:
        ...
        try:
            from fp_pg.cli import main as fp_main
            ...
            return {
                "success": False,
                "stdout": "",
                "stderr": "fp_pg module not available",
                "return_code": 1
            }
    ...
    def seek_query(self, args: List[str]) -> Dict[str, Any]:
        ...
        try:
            from seek_pg.cli import main as seek_main
            # This is a simplified approach - in practice, we'd need to properly 
            ...
            return {
                "success": False,
                "stdout": "",
                "stderr": "seek_pg module not available",
                "return_code": 1
            }
    ...
    def pt_query(self, args: List[str]) -> Dict[str, Any]:
        ...
        try:
            from pt_pg.cli import main as pt_main
            # This is a simplified approach - in practice, we'd need to properly 
            ...
            return {
                "success": False,
                "stdout": "",
                "stderr": "pt_pg module not available",
                "return_code": 1
            }
    
    def _get_pt_list(self) -> Dict[str, Any]:
        """Direct implementation of pt list functionality using PostgreSQL."""
        try:
            ...
            for entry in entries:
                ...
                result_text += "-" * 80 + "\n"
                result_text += f"ID: {entry_dict['id']}\n"
                result_text += f"Category: {entry_dict['category']}\n"
                result_text += f"Priority: {entry_dict['priority']}\n"
                result_text += f"Status: {entry_dict['status']}\n"
                result_text += f"Title: {entry_dict['title']}\n"
                desc = entry_dict['description'][:75] + "..." if len(str(entry_dict['description'])) > 75 else str(entry_dict['description'])
                result_text += f"Description: {desc}\n"
                result_text += f"Tags: {entry_dict['tags']}\n"
                result_text += f"Created: {entry_dict['created_date']}, Updated: {entry_dict['updated_date']}\n"
    ...
    def _search_pt_entries(self, search_term: str) -> Dict[str, Any]:
        """Direct implementation of pt search functionality using PostgreSQL."""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            
            # Search in title and description fields using ILIKE for PostgreSQL
            cursor.execute("""
                SELECT id, category, title, description, priority, status, tags, created_date, updated_date
                FROM project_tracker 
                WHERE project = 'openclaw' AND status != 'completed'
    ...
    def get_project_context(self, project_name: str) -> Dict[str, Any]:
        """Get context for a specific project by querying the PostgreSQL database directly."""
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            for entry in entries:
                ...
                result.append({
                    ...
                    "description": str(entry_dict['description'])[:100] + "..." if len(str(entry_dict['description'])) > 100 else str(entry_dict['description']),  # Truncate long descriptions
                    "priority": entry_dict['priority'],
                    "status": entry_dict['status'],
                    "tags": entry_dict['tags'],
                    "created_date": entry_dict['created_date'],
                    "updated_date": entry_dict['updated_date']
                })
    ...
    def _enhanced_fp_query(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use PostgreSQL connection for family planning data
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            try:
                # Search in tasks
                cursor.execute("SELECT id, action, category, due_date FROM family_tasks WHERE status != 'DONE' AND (action ILIKE %s OR category ILIKE %s)", (f'%{query}%', f'%{query}%'))
                task_results = cursor.fetchall()
                if task_results:
                    possible_matches.append(f"Family tasks related to '{query}': {len(task_results)} found")
                
                # Search in key dates
                cursor.execute("SELECT label, date, category FROM family_key_dates WHERE label ILIKE %s OR category ILIKE %s", (f'%{query}%', f'%{query}%'))
                date_results = cursor.fetchall()
                if date_results:
                    possible_matches.append(f"Family dates related to '{query}': {len(date_results)} found")
                    
                # Search in facts
                cursor.execute("SELECT topic, key, value FROM family_facts WHERE topic ILIKE %s OR key ILIKE %s OR value ILIKE %s", (f'%{query}%', f'%{query}%', f'%{query}%'))
    ...
    def get_recommendations(self, context: str = "") -> Dict[str, Any]:
        ...
        if pt_result["success"]:
            ...
            high_priority_items = [proj for proj in current_projects 
                                 if proj.get("priority") in ["critical", "high"] and proj.get("status") == "new"]
    ...
    def _initialize_knowledge_graph_db(self):
        """Initialize the knowledge graph database to store relationships between entities using PostgreSQL."""
        conn = self.get_connection()
        cursor = conn.cursor()
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_kg_source ON knowledge_graph_relationships(source_entity, source_type)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_kg_target ON knowledge_graph_relationships(target_entity, target_type)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_kg_relationship ON knowledge_graph_relationships(relationship_type)")
    ...
    def add_relationship(self, source_entity: str, source_type: str, target_entity: str, target_type: str, relationship_type: str, strength: float = 1.0) -> Dict[str, Any]:
        ...
        """
        try:
            self._initialize_knowledge_graph_db()
            conn = self.get_connection()
            cursor = conn.cursor()
            ...
            cursor.execute("""
                ...
                WHERE source_entity = %s AND source_type = %s AND target_entity = %s AND target_type = %s
            ...
            if existing:
                ...
                cursor.execute("""
                    ...
                    SET relationship_type = %s, strength = %s, updated_date = CURRENT_TIMESTAMP
                    WHERE id = %s
                """, (relationship_type, strength, existing[0]))
            else:
                # Insert new relationship
                cursor.execute("""
                    INSERT INTO knowledge_graph_relationships (source_entity, source_type, target_entity, target_type, relationship_type, strength)
                    VALUES (%s, %s, %s, %s, %s, %s)
    ...
    def get_related_entities(self, entity: str, entity_type: str = None, relationship_type: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            query_parts = ["SELECT source_entity, source_type, target_entity, target_type, relationship_type, strength FROM knowledge_graph_relationships WHERE source_entity = %s"]
            params = [entity]
            
            if entity_type:
                query_parts.append("AND source_type = %s")
                params.append(entity_type)
            
            if relationship_type:
                query_parts.append("AND relationship_type = %s")
                params.append(relationship_type)
            
            query_parts.append("UNION ALL")
            
            # Query relationships where the entity is the target
            query_parts.append("SELECT source_entity, source_type, target_entity, target_type, relationship_type, strength FROM knowledge_graph_relationships WHERE target_entity = %s")
            params.append(entity)
            
            if entity_type:
                query_parts.append("AND target_type = %s")
                params.append(entity_type)
    ...
    def automation_add_trigger(self, name: str, type: str, condition: Dict, action: Dict, enabled: bool = True) -> Dict[str, Any]:
        """Add a new trigger using the PostgreSQL TriggerManager."""
    ...
    def automation_list_triggers(self, enabled_only: bool = True) -> List[Dict[str, Any]]:
        """List triggers using the PostgreSQL TriggerManager."""
        if not self.trigger_manager:
            return [{"success": False, "error": "Trigger manager not available."}]
        return self.trigger_manager.list_triggers(enabled_only)

    def automation_check_triggers(self):
        """Check all triggers using the PostgreSQL TriggerManager."""
        if not self.trigger_manager:
            return {"success": False, "error": "Trigger manager not available."}
