#!/usr/bin/env python3
"""
Google Calendar CLI (gc)

Performs CRUD operations against Google Calendar.
Optionally syncs events to local database for tracking.
"""
...
class GoogleCalendarClient:
    ...
    def __init__(self, sync_to_local=False):
        self.service = None
        self.sync_to_local = sync_to_local
        self.local_db_path = os.path.expanduser("~/projects/_openclaw/calendar-sync.db")
        self._authenticate()
        if sync_to_local:
            self._init_local_db()
    
    def _init_local_db(self):
        """Initialize local database for calendar event syncing."""
    ...
    def _sync_event_to_local(self, event, calendar_id, operation):
        """Sync an event to the local database."""
        ...
        try:
            ...
            print(f"Event {event_id} {operation} and synced to local database.")
        except Exception as e:
            print(f"Error syncing event to local database: {e}")
    
    def _remove_event_from_local(self, event_id):
        """Remove an event from the local database."""
        if not self.sync_to_local:
            return
        ...
        try:
            ...
            print(f"Event {event_id} removed from local database.")
        except Exception as e:
            print(f"Error removing event from local database: {e}")
    
    def sync_local_events(self, calendar_id='primary'):
        """Sync local database events with Google Calendar."""
        if not self.service:
            print("Calendar service not available. Authentication failed.")
            return
        ...
        try:
            ...
            cursor.execute("SELECT * FROM calendar_events WHERE calendar_id = ?", (calendar_id,))
            local_events = cursor.fetchall()
            
            conn.close()
            
            print(f"Found {len(local_events)} events in local database for calendar '{calendar_id}'.")
...
def main():
    parser = argparse.ArgumentParser(description="Google Calendar CLI (gc) - Performs CRUD operations against Google Calendar")
    
    # Add global option for local sync
    parser.add_argument("--sync-local", action="store_true", help="Sync events to local database")
    ...
    parser_sync = subparsers.add_parser('sync-local', help='Sync local database events with Google Calendar')
